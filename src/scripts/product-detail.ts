...
async function render() {
  ...
  try {
    ...
    root.innerHTML = `<a class="mb-8 inline-flex items-center gap-2 text-sm font-bold text-ink/60 hover:text-candy" href="/shop"><i class="fi fi-rr-arrow-left"></i> Volver a la tienda</a><div class="grid gap-10 lg:grid-cols-2 lg:items-start"><div><div class="group overflow-hidden rounded-[2rem] bg-white shadow-candy"><img id="main-product-image" class="aspect-square w-full cursor-zoom-in object-cover transition duration-500 group-hover:scale-105" src="${product.image_url}" alt="${product.name}"/></div><div class="mt-4 grid grid-cols-4 gap-3">${gallery.map((image) => `<button class="product-thumb overflow-hidden rounded-2xl border-2 border-transparent transition hover:border-candy" data-image="${image}" type="button"><img class="aspect-square w-full object-cover" src="${image}" alt="${product.name}"/></button>`).join('')}</div></div><div class="lg:pt-10"><span class="inline-block rounded-full bg-bubblegum px-4 py-2 text-xs font-bold uppercase tracking-[0.14em]">${product.category}</span><h1 class="mt-5 font-display text-5xl font-bold tracking-[-0.08em] sm:text-7xl">${product.name}</h1><p class="mt-5 font-display text-3xl font-bold">C$${product.price}</p><p class="mt-5 text-lg leading-8 text-ink/65">${product.details || product.description}</p><dl class="mt-8 grid grid-cols-2 gap-4 border-y border-ink/10 py-5 text-sm"><div><dt class="text-ink/50">SKU</dt><dd class="mt-1 font-bold">${product.sku}</dd></div><div><dt class="text-ink/50">Disponibles</dt><dd class="mt-1 font-bold">${product.stock} unidades</dd></div></dl><div class="mt-8 flex gap-3"><div class="flex items-center rounded-full border border-ink/15"><button class="detail-minus grid size-12 place-items-center" type="button">−</button><span id="detail-quantity" class="w-6 text-center font-bold">1</span><button class="detail-plus grid size-12 place-items-center" type="button">+</button></div><button id="detail-add" class="candy-button-pink flex-1" type="button" data-product-id="${product.id}">Añadir al carrito <i class="fi fi-rr-shopping-bag"></i></button></div></div></div><section class="mt-24 border-t border-ink/10 pt-12"><p class="text-xs font-bold tracking-[0.22em] text-candy">YOU MAY ALSO LIKE</p><h2 class="mt-2 font-display text-4xl font-bold tracking-[-0.07em]">Más como esto</h2><div class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">${related.map(relatedCard).join('')}</div></section>`;
